        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                @forelse($jobs as $i=> $job)
                    <tr @if (is_array($selections)) @if(in_array($job->id, $selections))  @endif @endif wire:key="row-{{ $job->id }}" class="text-gray-700 dark:text-gray-400">
                        <td><input type="checkbox" value="{{ $job->id }}" wire:model="selections"></td>
                        <td class="px-4 py-3">
                            <div class="flex items-center text-sm">
                                ...
                                >
                                    <img
                                        ...
                                        src="https://www.gravatar.com/avatar/{{md5($job->email)}}?d=mp"
                                        alt=""
                                        loading="lazy"
                                    />
                                    <div
                        ...
                        <td class="px-4 py-3 text-sm">
                            <span class="uppercase px-2 py-1 leading-tight {{$job->type==='mosque'?'text-blue-700 bg-green-100':'text-purple-700 bg-purple-100'}}  rounded-full ">{{$job->type}}</span> </td>
                        <td class="px-4 py-3 text-sm">
                            <span class="uppercase px-2 py-1 leading-tight {{$job->sex==='male'?'text-blue-700 bg-green-100':'text-purple-700 bg-purple-100'}}  rounded-full ">{{$job->sex}}</span></td>
                        <td class="px-4 py-3 text-xs">
                            <button class="uppercase px-2 py-1 leading-tight {{$job->request_status==='requested'?'text-green-700 bg-green-100':'text-red-700 bg-red-100'}}  rounded-full " wire:click.prevent="updateRequestStatus({{ $job->id }})">{{$job->request_status}}
                                <span wire:loading wire:target="updateRequestStatus({{ $job->id }})" class="animate-spin rounded-full h-4 w-4 border-2 border-black"></span></button>
                        </td>
                        <td class="px-4 py-3 text-xs">
                            <button class="uppercase px-2 py-1 leading-tight {{$job->status==='active'?'text-green-700 bg-green-100':'text-red-700 bg-red-100'}}  rounded-full " wire:click.prevent="updateStatus({{ $job->id }})">{{ $job->status }}
                                <span wire:loading wire:target="updateStatus({{ $job->id }})" class="animate-spin rounded-full h-4 w-4 border-2 border-black"></span></button>
                        </td>
                        <td class="px-4 py-3 text-xs">{{\Carbon\Carbon::parse($job->created_at)->diffForHumans()}}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                ...
                                <a wire:click.prevent="confirmDeletion({{ $job->id }})"
                ...
                @empty
                    <center> <h2 class="text-red-600">No job found</h2> </center>
                @endforelse
                </tbody>
            </table>
        </div>
