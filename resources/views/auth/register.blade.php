        ...
        >
            <div class="flex flex-col overflow-y-auto md:flex-row">
                <div class="h-32 md:h-auto md:w-1/2">
                    <img src="{{\App\Models\Setup::first()->getFirstMediaUrl('register')}}" aria-hidden="true" onerror="this.src='https://i.ytimg.com/vi/mtXQ-m2xPEY/maxresdefault.jpg'" alt="cover" class="object-cover w-full h-full">
                </div>
                <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                    @if (session('status'))
                        <div class="form-group">{{ session('status') }}</div>
                    @endif
                    <form method="POST" action="{{ route('register') }}" class="w-full"> @csrf
                        ...
                        <label class="block mt-4 text-sm">
                            ...
                            <input
                                ...
                                type="password" placeholder="{{__('Enter your password')}}" name="password" required autocomplete="new-password"/>
                            @error('password')<span class="text-sm text-red-600 dark:text-red-400">{{ $message }}</span>@enderror
                        </label>
                        <label class="block mt-4 text-sm">
                            <span class="text-gray-700 dark:text-gray-400">{{__('Enter your password again')}}</span>
                            <input
                                class="form-control-tw @error('password_confirmation') is-invalid @enderror form-input"
                                type="password" placeholder="{{__('Enter your password again')}}" name="password_confirmation" required autocomplete="new-password"/>
                        ...
                        <div class="flex mt-3 text-sm">
                            <label for="remember_me" class="flex items-center dark:text-gray-400">
                                <input name="terms" id="terms" type="checkbox" class="text-purple-600 form-checkbox bg-gray-300 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray @error('terms') is-invalid @enderror""/>
                                <span class="ml-2">{{ __('I accept the Terms of Services') }}</span>
                    ...
                    <!-- You should use a button here, as the anchor is only used for the example  -->
                        <button type="submit"
                                class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">{{__('Register')}}
                        </button>
                        {{--                        <hr class="my-8" />--}}
